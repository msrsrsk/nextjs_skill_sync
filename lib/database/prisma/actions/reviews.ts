...
interface GetPaginatedReviewsDataProps {
    page: number;
    limit: number;
    query?: string;
    userId?: UserId;
...
// レビューの作成
export const createReviewData = async ({ reviewData }: { reviewData: Review }) => {
...
export const getSectionReviewsData = async () => {
    // ユーザーデータがあるレビュー
...
// 個別商品のレビューの取得
export const getProductReviewsData = async ({ 
    productSlug 
...
// レビューのページネーション付きデータの取得
export const getPaginatedReviewsData = async ({ 
    page, 
    limit, 
    query,
    userId
}: GetPaginatedReviewsDataProps) => {
