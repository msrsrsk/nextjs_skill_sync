...
describe("formatHtmlToPlainText", () => {
  ...
  it("should remove HTML tags", () => {
    ...
    expect(formatHtmlToPlainText('<span class="test">Content</span>')).toBe(
      "Content",
    );
  });
  ...
  it("should remove multiple HTML tags", () => {
    expect(formatHtmlToPlainText("<p>Hello <strong>World</strong></p>")).toBe(
      "Hello World",
    );
    expect(
      formatHtmlToPlainText("<div><span>Test</span> <em>Content</em></div>"),
    ).toBe("Test Content");
  });
  ...
  it("should convert &nbsp; to space", () => {
    ...
    expect(formatHtmlToPlainText("Test&nbsp;&nbsp;Content")).toBe(
      "Test Content",
    );
  });
  ...
  it("should replace multiple spaces with single space", () => {
    ...
    expect(formatHtmlToPlainText("Multiple     Spaces")).toBe(
      "Multiple Spaces",
    );
  });
  ...
  it("should handle nested HTML tags", () => {
    expect(formatHtmlToPlainText("<div><p><span>Nested</span></p></div>")).toBe(
      "Nested",
    );
  ...
  it("should handle mixed HTML and plain text", () => {
    expect(formatHtmlToPlainText("Start <p>Middle</p> End")).toBe(
      "Start Middle End",
    );
    expect(formatHtmlToPlainText("Text <strong>Bold</strong> More Text")).toBe(
      "Text Bold More Text",
    );
  });
});
...
describe("formatDateCommon", () => {
  ...
  it("should format dates from different years", () => {
    ...
    expect(formatDateCommon(new Date(2024, 5, 15), DATE_DOT)).toBe(
      "2024.06.15",
    );
    expect(formatDateCommon(new Date(2100, 11, 31), DATE_DOT)).toBe(
      "2100.12.31",
    );
  });
