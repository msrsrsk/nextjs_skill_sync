...
Prisma と PostgreSQL を用いたデータ管理の方法も合わせて今回チャレンジすることにしました。<br />
...
| アナザーセクション                                                                                                                  | ミニゲーム                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| [![Image from Gyazo](https://i.gyazo.com/c1a04f35953fc79a913079ac20fc1fd1.png)](https://gyazo.com/c1a04f35953fc79a913079ac20fc1fd1) | [![Image from Gyazo](https://i.gyazo.com/1be4f15cb925220aefcd1f5035ac1dcd.png)](https://gyazo.com/1be4f15cb925220aefcd1f5035ac1dcd) |
| BlenderとThree.jsで作成した簡易コンフィグレーター一覧を掲載しています。                                                             | フッターにある「play a bit」ボタンから、BlenderとThree.jsで作成したミニゲームで遊ぶことができます。                                 |

<div>
  <br />
</div>

## <a name="database">📮 データベース構成</a>

本プロジェクトのデータ層は **Prisma + PostgreSQL** を中心に構成しています。

| レイヤー     | 技術                       | 役割                                                              |
| ------------ | -------------------------- | ----------------------------------------------------------------- |
| ORM          | Prisma                     | 型安全な DB アクセス・マイグレーション                            |
| データベース | PostgreSQL                 | ユーザー・商品・注文などアプリデータの永続化                      |
| Webhook      | Supabase Database Webhooks | DB 変更時の外部連携トリガー（商品 Stripe 連携・レビュー通知など） |

- アプリケーションからの DB 操作は **Prisma 経由のみ**（`@supabase/supabase-js` は使用していません）
- 認証は **Auth.js (NextAuth.js)** + Prisma Adapter
- Supabase は **Webhook 認証**（`/api/webhook/product` など）に利用しています

<a href="https://github.com/msrsrsk/nextjs_skill_sync/blob/main/prisma/ERD.md" target="_blank" rel="noopener noreferrer">
ER 図はこちら
</a>
...
| Back-end           | Next.js API Routes, Prisma, PostgreSQL, Stripe, Resend, OpenAI                                       |
| Authentication     | Auth.js (NextAuth.js) + Prisma Adapter                                                               |
| Content Management | microCMS                                                                                             |
| Infrastructure     | PostgreSQL, Supabase (Database Webhooks), Cloudflare R2 (Storage), Vercel (Hosting)                  |
...
Prisma と PostgreSQL を用いたデータ管理から Stripe の決済機能など、実際に設計から実装まで手を動かし、<br />
