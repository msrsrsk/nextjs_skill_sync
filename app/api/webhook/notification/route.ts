...
import { verifyDatabaseWebhookAuth } from "@/lib/utils/webhook";
...
export async function POST(request: NextRequest) {
  try {
    const webhookData = await verifyDatabaseWebhookAuth({
      request,
    ...
    const errorMessage =
      error instanceof Error ? error.message : WEBHOOK_ERROR.PROCESS_FAILED;
