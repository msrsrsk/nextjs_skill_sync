...
interface FaqItemProps {
  itemKey: string;
  isExpanded: boolean;
  toggleAccordion: (categoryIndex: number, faqIndex: number) => void;
    ...
    <article key={itemKey} className="faq-box">
      ...
      >
        <div className="faq-question__inner">
          ...
          <p className="faq-question__text">{faq.question}</p>
        </div>
        <ChevronDown
          className={`faq-toggle${isExpanded ? " rotate-180" : ""}`}
        />
      </button>
      <motion.div
        id={`faq-${itemKey}`}
