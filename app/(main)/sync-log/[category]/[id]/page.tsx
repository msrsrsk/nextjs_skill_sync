...
const SyncLogDetailPage = async ({ params }: { params: { id: string } }) => {
  ...
  const { imageUrl, categoryName, productName } =
    extractSyncLogData(productLink);

  const LinkimageUrl = imageUrl
    ? `${PRODUCT_PATH}/${imageUrl}.png`
    : NOIMAGE_PRODUCT_IMAGE_URL;

  const isCategoryCondition =
    data.category.name === SYNC_UPDATES || data.category.name === SYNC_VOICES;
                ...
                {isCategoryCondition && (
                  <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6">
                    ...
                    <div>
                      ...
                      <h3 className="product-title mb-4">{productName}</h3>
