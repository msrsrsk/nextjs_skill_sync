...
export async function generateMetadata({
  ...
  return generatePageMetadata({
    title: "トレンド商品一覧",
    description:
      "トレンド商品の一覧ページです。月間100万個以上の購入があるトレンド商品が掲載されます。",
    url: `${TREND_PATH}/${category.toLowerCase()}`,
  });
}

const TrendCategoryPage = ({ params, searchParams }: CategoryPageProps) => {
  const page = parseInt(searchParams.page || DEFAULT_PAGE);
  const category = params.category;

  const categoryTitle = formatTitleCase(category);
  ...
  return (
    <>
      ...
      <div className="c-container-page-overflow">
        ...
        />
        <p className="page-subtitle">世界で人気のトレンドを装備する</p>
