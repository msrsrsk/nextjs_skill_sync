...
export async function generateMetadata({
  ...
  return generatePageMetadata({
    title: "サブスクリプション詳細",
    description:
      "お客様のサブスクリプションのご契約の詳細を確認することができます。",
...
const SubscriptionHistoryDetailPage = async ({
  params,
}: {
  params: { id: OrderId };
}) => {
  ...
  const subscriptionId =
    order_items[0].order_item_subscriptions?.subscription_id;
  const subscriptionStatus = order_items[0].order_item_subscriptions?.status;
  const nextPaymentDate =
    order_items[0].order_item_subscriptions?.next_payment_date;
        ...
        <div className="order-wrapper">
          <div className="order-inner">
            ...
            <div className="order-title-box">
              <h4 className="order-title">サブスクリプションの詳細</h4>
              ...
              <div className="lg:max-w-[440px] w-full">
                ...
                <div className="w-full">
                  <dl className="order-info-dl">
                    ...
                    <OrderInfoDisplay
                      label="次回支払日　"
                      customClass={`${isActive ? "font-poppins" : ""}${
                        isCanceled ? " text-tag-default" : ""
                      }`}
                    >
                      {isCanceled ? "解約済み" : formattedNextPaymentDate}
                    </OrderInfoDisplay>
                    <OrderInfoDisplay label="注文状況　　">
                      <OrderStatus status={status} type={ORDER_DETAIL} />
                    </OrderInfoDisplay>
