...
const DeleteAccountWrapper = async () => {
  ...
  const unshippedOrdersCount = await repository.getUnshippedOrdersCount({
    userId,
  });

  if (unshippedOrdersCount === null || unshippedOrdersCount === undefined)
    return (
      <ErrorMessage message={USER_ERROR.UNSHIPPED_ORDERS_COUNT_MISSING_DATA} />
    );

  return <DeleteAccountContent unshippedOrdersCount={unshippedOrdersCount} />;
};
