...
export const createCheckoutOrder = async ({
  ...
  const paymentStatus =
    session.payment_status === "paid" ? ORDER_PROCESSING : ORDER_PENDING;
  ...
  try {
    ...
    const createOrderShippingResult = await createOrderShipping({
      orderShippingData: {
        ...
        shipping_fee:
          subscriptionShippingFee || session.shipping_cost?.amount_total || 0,
      },
    });
...
export const updateProductStockAndSoldCount = async ({
  ...
  try {
    ...
    const { success, error } = await updateStockAndSoldCount({
      productUpdates,
    });
    ...
    console.error(
      "Database : Error in updateProductStockAndSoldCount: ",
      error,
    );
