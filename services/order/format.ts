...
export const formatPaymentMethodType = (paymentMethodType: string): string => {
  ...
  return (
    methodMap[paymentMethodType as keyof typeof methodMap] || paymentMethodType
  );
};
...
export const formatOrderDateTime = (
  ...
  const minutes = String(convertedDate.getMinutes()).padStart(
    PADDING_LENGTH,
    "0",
  );
...
export const getPaymentErrorDetails = (errorType: string): string => {
  ...
  return errorKey
    ? ERROR_MESSAGES.PAYMENT_ERROR[errorKey]
    : ERROR_MESSAGES.PAYMENT_ERROR.UNKNOWN;
};
