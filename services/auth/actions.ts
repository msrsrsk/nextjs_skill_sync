  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (!deleteResult.success) {
        console.error(
          "Actions Error - Delete Verification Token failed: ",
          deleteResult,
        );
      }
...
export async function createVerificationTokenWithPassword(
  ...
  try {
    ...
    console.error(
      "Actions Error - Create Verification Token With Password error:",
      error,
    );
...
export async function verifyEmailToken(
  ...
  try {
    // 1. 認証トークンの取得&検証
    const { success: verificationSuccess, data: verificationToken } =
    ...
    const tokenUserData =
      verificationToken.userData && JSON.parse(verificationToken.userData);
    ...
    if (verifyEmailType === VERIFY_CREATE_ACCOUNT) {
      ...
      // 4. PrismaユーザーのStripe顧客IDの作成
      const { success: createCustomerIdSuccess, error: createCustomerIdError } =
...
export async function createVerificationTokenWithEmail(email: UserEmail) {
  ...
  try {
    ...
    console.error(
      "Actions Error - Create Email Verification Token With Email error:",
      error,
    );
