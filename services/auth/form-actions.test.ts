...
const {
  ...
  CREATE_ACCOUNT_MISSING_DATA,
  EMAIL_MISSING_DATA,
  RESET_PASSWORD_PROCESS_FAILED,
  SESSION_NOT_FOUND,
  PASSWORD_MISSING_DATA,
  TOKEN_MISSING_DATA,
  USER_NOT_FOUND,
  EMAIL_NOT_MATCH,
  INCORRECT_EMAIL_OR_PASSWORD,
  SIGN_IN_MISSING_DATA,
} = AUTH_ERROR;
const { AUTH_SEND_FAILED } = EMAIL_ERROR;
const { EMAIL_RESET_PASSWORD_PAGE, EMAIL_UPDATE_EMAIL_PAGE } =
  EMAIL_VERIFICATION_PAGE_TYPES;
...
const getMockCreateVerificationTokenWithPassword = async () => {
  const { createVerificationTokenWithPassword } =
    await import("@/services/auth/actions");
  return vi.mocked(createVerificationTokenWithPassword);
};

const getMockCreateVerificationTokenWithEmail = async () => {
  const { createVerificationTokenWithEmail } =
    await import("@/services/auth/actions");
...
const getMockSendVerificationEmail = async () => {
  const { sendVerificationEmail } =
    await import("@/services/email/auth/verification");
...
describe("createAccountWithEmailAction", () => {
  ...
  it("should create account with email successfully", async () => {
    const mockCreateVerificationTokenWithPassword =
      await getMockCreateVerificationTokenWithPassword();
    ...
    const result = await createAccountWithEmailAction(
      commonPrevState,
      commonFormData,
    );
  ...
  it("should return failure when send verification email throws an exception", async () => {
    ...
    mockSendVerificationEmail.mockRejectedValue(new Error("Database error"));
...
describe("sendVerificationEmailAction", () => {
  ...
  it("should send verification email successfully when user exists", async () => {
    const mockCreateVerificationTokenWithEmail =
      await getMockCreateVerificationTokenWithEmail();
    const mockSendVerificationEmail = await getMockSendVerificationEmail();
    ...
    expect(mockCreateVerificationTokenWithEmail).toHaveBeenCalledWith(
      commonEmail,
    );
  ...
  it("should return failure when sendVerificationEmail throws an exception", async () => {
    ...
    mockSendVerificationEmail.mockRejectedValue(new Error("Email send error"));
