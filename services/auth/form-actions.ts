...
const { CREATE_ACCOUNT_TYPE, RESET_PASSWORD_TYPE, UPDATE_EMAIL_TYPE } =
  EMAIL_VERIFICATION_TYPES;
...
export async function createAccountWithEmailAction(
  ...
  try {
    ...
    const {
      ...
      token,
    } = await createVerificationTokenWithPassword(userData);
    ...
    // 3. 認証メールの送信
    const { success: sendVerificationSuccess, error: sendVerificationError } =
    ...
    const errorMessage =
      error instanceof Error ? error.message : AUTH_ERROR.CREATE_ACCOUNT_FAILED;
...
export async function sendVerificationEmailAction(
  ...
  const emailType =
    type === EMAIL_RESET_PASSWORD_PAGE
      ? RESET_PASSWORD_TYPE
      : UPDATE_EMAIL_TYPE;
  ...
  try {
    ...
    // 2. 認証トークンの作成
    const { success: tokenSuccess, token } =
    ...
    // 3. 認証メールの送信
    const { success: sendMailSuccess, error: sendMainError } =
      await sendVerificationEmail({
        email,
        token,
        type: emailType,
    ...
    const errorMessage =
      error instanceof Error ? error.message : AUTH_ERROR.TOKEN_SEND_FAILED;
...
export async function updatePasswordAction(
  ...
  const token =
    type === RESET_PASSWORD_PAGE ? (formData.get("token") as string) : "";
  ...
  try {
    ...
    if (type === RESET_PASSWORD_PAGE) {
      const { success, error } = await updatePasswordWithToken(token, password);
