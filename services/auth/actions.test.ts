...
const {
  ...
  SESSION_NOT_FOUND,
  CREATE_VERIFICATION_TOKEN_PROCESS_FAILED,
} = AUTH_ERROR;
const {
  NOT_FOUND_EMAIL_TOKEN,
  EXPIRED_EMAIL_TOKEN,
  VERIFY_TOKEN_FAILED,
  NOT_FOUND_PASSWORD_TOKEN,
...
const getMockDeleteVerificationToken = async () => {
  const { deleteVerificationToken } =
    await import("@/services/verification-token/actions");
...
const getMockGetVerificationTokenAndVerify = async () => {
  const { getVerificationTokenAndVerify } =
    await import("@/services/verification-token/actions");
...
const getMockCreateUserStripeCustomerId = async () => {
  const { createUserStripeCustomerId } =
    await import("@/services/user-stripe/actions");
...
const getMockUpdateUserPasswordWithTransaction = async () => {
  const { updateUserPasswordWithTransaction } =
    await import("@/services/user/actions");
...
describe("verifyEmailToken", () => {
  ...
  it("should verify email token successfully when verifyEmailType is VERIFY_CREATE_ACCOUNT", async () => {
    const mockGetVerificationTokenAndVerify =
      await getMockGetVerificationTokenAndVerify();
    const mockCreateStripeCustomer = await getMockCreateStripeCustomer();
    const mockCreateUserStripeCustomerId =
      await getMockCreateUserStripeCustomerId();
  ...
  it("should return failure when hashedPassword is null", async () => {
    ...
    const result = await verifyEmailToken(
      verificationTokenWithoutPassword.token,
      VERIFY_CREATE_ACCOUNT,
    );
  ...
  it("should return failure when register user with chat fails", async () => {
    ...
    mockDeleteVerificationToken.mockRejectedValue(
      new Error(CREATE_ACCOUNT_PROCESS_FAILED),
    );
  ...
  it("should return failure when verificationToken.identifier is null", async () => {
    ...
    const result = await verifyEmailToken(
      verificationTokenWithNullIdentifier.token,
      VERIFY_CREATE_ACCOUNT,
    );
  ...
  it("should handle when verificationToken.userData is null", async () => {
    ...
    const result = await verifyEmailToken(
      verificationTokenWithNullUserData.token,
      VERIFY_CREATE_ACCOUNT,
    );

    expect(result.success).toBe(false);
    expect(result.error).toBe(
      "Cannot read properties of null (reading 'lastname')",
    );
    expect(result.expires).toBeNull();
  });
  ...
  it("should return failure when create stripe customer throws an exception", async () => {
    ...
    mockCreateStripeCustomer.mockRejectedValue(new Error("Database error"));
  ...
  it("should return failure when delete stripe customer throws an exception", async () => {
    ...
    mockDeleteStripeCustomer.mockRejectedValue(new Error("Database error"));
  ...
  it("should return failure when actionAuth throws an exception", async () => {
    ...
    mockActionAuth.mockRejectedValue(new Error("Database error"));
...
describe("resetPassword", () => {
  ...
  it("should reset password successfully", async () => {
    const mockUpdateUserPasswordWithTransaction =
      await getMockUpdateUserPasswordWithTransaction();
    const mockDeleteVerificationToken = await getMockDeleteVerificationToken();
