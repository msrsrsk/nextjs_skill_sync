...
export const uploadImageToR2 = async ({
  ...
  try {
    ...
    for (const file of files) {
      const { success, error, data } = await uploadSingleFile({
        file,
        userImageId,
        bucketType,
        userId,
...
export const getAuthenticatedProfileImageUrl = async ({
  ...
  try {
    // 1. ユーザー画像の認証と権限確認
    const { success, error, data } = await authenticateAndAuthorizeUserImage({
      userImageId,
    });
    ...
    console.error(
      "Storage Error - Get Authenticated Profile Image URL error:",
      error,
    );

    const errorMessage =
      error instanceof Error ? error.message : CLOUDFLARE_ERROR.FETCH_FAILED;
