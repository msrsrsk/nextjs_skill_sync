...
const { CLOUDFLARE_ERROR, USER_IMAGE_ERROR, USER_PROFILE_ERROR } =
  ERROR_MESSAGES;

const { R2_UPLOAD_FAILED, FETCH_FAILED, PROFILE_ACCESS_DENIED, DELETE_FAILED } =
  CLOUDFLARE_ERROR;
const { FILE_PATH_UPDATE_FAILED, USER_REQUIRED_DATA_NOT_FOUND } =
  USER_IMAGE_ERROR;
...
vi.mock("@/repository/userImage", () => ({
  getUserImageRepository: () => ({
    ...
    getUserImageFilePathWithTransaction:
      mockGetUserImageFilePathWithTransaction,
  }),
}));
...
const getUpdateUserImageFilePath = async () => {
  const { updateUserImageFilePath } =
    await import("@/services/user-image/actions");
...
const getUploadImageToR2 = async () => {
  const { uploadImageToR2 } =
    await import("@/services/cloudflare/internal-actions");
  return uploadImageToR2;
};

const getGetAuthenticatedProfileImageUrl = async () => {
  const { getAuthenticatedProfileImageUrl } =
    await import("@/services/cloudflare/internal-actions");
  ...
  it("should upload review image successfully", async () => {
    ...
    expect(result).toEqual({
      ...
      data: expect.stringContaining(
        `https://example.com/${BUCKET_REVIEW}/${mockUserImageId}/`,
      ),
    });
  ...
  it("should upload profile image successfully", async () => {
    ...
    expect(updateUserImageFilePath).toHaveBeenCalledWith({
      ...
      filePath: expect.stringContaining(
        `${BUCKET_PROFILE}/${mockUserImageId}/`,
      ),
    });
  });
...
describe("uploadImageIfNeeded", () => {
  ...
  it("should upload data URL and return authenticated URL", async () => {
    ...
    const getAuthenticatedProfileImageUrl =
      await getGetAuthenticatedProfileImageUrl();
  ...
  it("should handle getAuthenticatedProfileImageUrl exception error", async () => {
    ...
    vi.mocked(getAuthenticatedProfileImageUrl).mockRejectedValue(
