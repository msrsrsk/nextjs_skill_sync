...
const { R2_UPLOAD_PROCESS_FAILED, MISSING_DATA, AUTHENTICATION_FAILED } =
  CLOUDFLARE_ERROR;
...
const getAuthenticateAndAuthorizeUserImage = async () => {
  const { authenticateAndAuthorizeUserImage } =
    await import("@/services/cloudflare/actions");
...
describe("getAuthenticatedProfileImageUrl", () => {
  ...
  it("should get authenticated profile image url successfully", async () => {
    ...
    const authenticateAndAuthorizeUserImage =
      await getAuthenticateAndAuthorizeUserImage();
    ...
    const getObjectOutput: MockGetObjectOutput = {
      Body: {
        transformToByteArray: async () => Buffer.from("abc123", "base64"),
      },
