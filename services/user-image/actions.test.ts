...
const { USER_REQUIRED_DATA_NOT_FOUND, CREATE_FAILED, FILE_PATH_UPDATE_FAILED } =
  USER_IMAGE_ERROR;
...
describe("deleteExistingImage", () => {
  ...
  it("should delete existing image successfully", async () => {
    ...
    expect(mockGetUserImageId).toHaveBeenCalledWith({
      userId: commonParams.userId,
    });
    expect(mockDeleteProfileImage).toHaveBeenCalledWith({
      userImageId: "test-user-image-id",
    });
  });
  ...
  it("should return failure when user image id is not found", async () => {
    ...
    await expect(deleteExistingImage(commonParams)).rejects.toThrow(
      USER_REQUIRED_DATA_NOT_FOUND,
    );
  ...
  it("should handle delete profile image failure gracefully", async () => {
    ...
    await expect(deleteExistingImage(commonParams)).resolves.toBeUndefined();
