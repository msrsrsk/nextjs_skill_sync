...
describe("extractProductLink", () => {
  ...
  it("should extract link when attributes are in different order", () => {
    const content =
      '<a class="btn" href="https://example.com/product/456" id="link">商品を見る</a>';
    const result = extractProductLink(content);
  ...
  it("should extract link with multiple attributes", () => {
    const content =
      '<a href="https://example.com/product/789" class="product-link" target="_blank" rel="noopener">商品を見る</a>';
    const result = extractProductLink(content);
