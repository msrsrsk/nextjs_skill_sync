...
// 商品詳細ページのデータ取得
export const getProductBySlug = async ({ slug }: { slug: ProductSlug }) => {
  try {
    ...
    const requiredIds = parseIds(
      productRelations?.optimal_syncs_required_id || "",
    );
    const optionIds = parseIds(productRelations?.optimal_syncs_option_id || "");
    const recommendedIds = parseIds(
      productRelations?.optimal_syncs_recommended_id || "",
    ...
    const optionLimited = collectUniqueIds(
      optionIds,
      maxCount - uniqueIds.size,
    );
    const recommendedLimited = collectUniqueIds(
      recommendedIds,
      maxCount - uniqueIds.size,
    );
