...
const { NOT_FOUND, FETCH_FAILED, UPDATE_STOCK_AND_SOLD_COUNT_FAILED } =
  PRODUCT_ERROR;
...
describe("getProductBySlug", () => {
  ...
  it("should return product data with optimal syncs successfully", async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.data?.optimalSyncs?.requiredIds).toEqual([
      "id1",
      "id2",
      "id3",
    ]);
  ...
  it("should respect OPTIMAL_SYNCS_LIMIT", async () => {
    ...
    expect(result.data?.optimalSyncs?.requiredIds.length).toBeLessThanOrEqual(
      OPTIMAL_SYNCS_LIMIT,
    );
  });
  ...
  it("should return error when exception occurs", async () => {
    mockGetProductBySlug.mockRejectedValue(
...
describe("updateStockAndSoldCount", () => {
  ...
  it("should update stock and sold count successfully for single product", async () => {
    const mockProductUpdates = [{ productId: "product_123", quantity: 5 }];
    ...
    const result = await updateStockAndSoldCount({
      productUpdates: mockProductUpdates,
    });
  ...
  it("should handle error in product stock update", async () => {
    ...
    const mockUpdateProductStock = vi
      .fn()
      .mockRejectedValue(new Error("Stock update failed"));
    const mockUpdateProductSoldCount = vi.fn().mockResolvedValue(undefined);
  ...
  it("should handle error in product sold count update", async () => {
    ...
    const mockUpdateProductSoldCount = vi
      .fn()
      .mockResolvedValue(new Error("Sold count update failed"));
