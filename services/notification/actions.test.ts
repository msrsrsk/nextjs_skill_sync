...
const getNotificationRepository = async () => {
  const { getNotificationRepository } =
    await import("@/repository/notification");
  return vi.mocked(getNotificationRepository);
};

const getMockReceiveStockNotificationEmail = async () => {
  const { receiveStockNotificationEmail } =
    await import("@/services/email/notification/stock");
  return vi.mocked(receiveStockNotificationEmail);
};

const getMockReceiveChatNotificationEmail = async () => {
  const { receiveChatNotificationEmail } =
    await import("@/services/email/notification/chat");
...
describe("processNotificationWebhook", () => {
  ...
  it("should process product_stock notification successfully", async () => {
    const mockReceiveStockNotificationEmail =
      await getMockReceiveStockNotificationEmail();

    mockGetNotificationWithDetails.mockResolvedValue(
      mockProductStockNotificationWithDetails,
    );
    ...
    expect(mockGetNotificationWithDetails).toHaveBeenCalledWith(
      mockProductStockRecord,
    );
    expect(mockReceiveStockNotificationEmail).toHaveBeenCalledWith(
      mockProductStockNotificationWithDetails,
    );
  ...
  it("should process chat notification successfully", async () => {
    const mockReceiveChatNotificationEmail =
      await getMockReceiveChatNotificationEmail();

    mockGetNotificationWithDetails.mockResolvedValue(
      mockChatNotificationWithDetails,
    );
    ...
    expect(mockReceiveChatNotificationEmail).toHaveBeenCalledWith(
      mockChatNotificationWithDetails,
    );
    expect(result.success).toBe(true);
    expect(result.error).toBeUndefined();
  ...
  it("should process notification webhook failed with record is null", async () => {
    const result = await processNotificationWebhook({
      record: null as unknown as NotificationData,
    });
    expect(result.success).toBe(false);
  ...
  it("should process notification webhook failed with unknown type", async () => {
    ...
    const result = await processNotificationWebhook({
      record: mockUnknownNotificationRecord,
    });
