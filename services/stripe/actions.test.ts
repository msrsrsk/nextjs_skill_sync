...
const { STRIPE_ERROR, SHIPPING_ADDRESS_ERROR, SUBSCRIPTION_ERROR } =
  ERROR_MESSAGES;
...
const getMockUpdateOrderItemSubscriptionStatus = async () => {
  const { updateOrderItemSubscriptionStatus } =
    await import("@/services/order-item-subscription/actions");
...
describe("createStripeProduct", () => {
  ...
  it("should return error when creating stripe product fails", async () => {
    ...
    vi.mocked(stripe.products.create).mockRejectedValue(
...
describe("createStripePrice", () => {
  ...
  it("should return error when creating stripe price fails", async () => {
    ...
    vi.mocked(stripe.prices.create).mockRejectedValue(
      new Error("Stripe API Error"),
    );
...
describe("createSubscriptionPrices", () => {
  ...
  it("should create subscription prices successfully", async () => {
    const mockExtractCreateSubscriptionPrices = vi.mocked(
      extractCreateSubscriptionPrices,
    );
    const mockFormatCreateSubscriptionNickname = vi.mocked(
      formatCreateSubscriptionNickname,
    );
    const mockGetRecurringConfig = vi.mocked(getRecurringConfig);
    const mockExtractUpdatedSubscriptionPriceIds = vi.mocked(
      extractUpdatedSubscriptionPriceIds,
    );
    ...
    mockGetRecurringConfig.mockReturnValue({
      interval: "day",
      interval_count: 1,
    });
  ...
  it("should return subscription price ids when recurring config is null", async () => {
    ...
    mockExtractUpdatedSubscriptionPriceIds.mockReturnValue(
      subscriptionPriceIds,
    );
...
describe("cancelSubscription", () => {
  ...
  it("should cancel subscription successfully", async () => {
    ...
    const updateOrderItemSubscriptionStatus =
      await getMockUpdateOrderItemSubscriptionStatus();
  ...
  it("should return error when payment_link is undefined", async () => {
    ...
    const result = await deactivatePaymentLink({
      checkoutSessionEvent: {
        ...mockCheckoutSession,
        payment_link:
          undefined as unknown as StripeCheckoutSession["payment_link"],
      },
    });
