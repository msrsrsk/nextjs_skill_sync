...
const { SHIPPING_ADDRESS_ERROR, SUBSCRIPTION_ERROR, STRIPE_ERROR } =
  ERROR_MESSAGES;
...
export const createSubscriptionPrices = async ({
  ...
  try {
    const subscriptionPrices =
      extractCreateSubscriptionPrices(subscriptionPriceIds);
    ...
    const subscriptionPriceResults = await Promise.all(
      subscriptionPrices.map(async (priceData) => {
        ...
        if (!recurringConfig) {
          console.error(
            SUBSCRIPTION_ERROR.SUBSCRIPTION_PRICE_RECURRING_CONFIG_FETCH_FAILED,
          );
          return null;
        }
    ...
    const successfulPrices = subscriptionPriceResults.filter(
      (result) => result !== null,
    );
...
export const updateCustomerShippingAddress = async (
  ...
  try {
    ...
    console.error(
      "Actions Error - Update Customer Shipping Address error:",
      error,
    );
