...
} from "@/services/stripe/checkout-helpers";
import {
  SITE_MAP,
...
const { USER_STRIPE_ERROR, CHECKOUT_ERROR } = ERROR_MESSAGES;
...
export const getSubscriptionShippingFee = async ({
  ...
  try {
    ...
    if (subscription.latest_invoice) {
      ...
      subscriptionShippingFee = Number(
        subscriptionMetadata?.subscription_shipping_fee,
      );
...
export const processCheckoutItems = async ({
  ...
  if (
    !calculateCheckoutTotalsResult.success ||
  ...
  const { lineItems, serverCalculatedTotal, dbBasedTotal } =
    calculateCheckoutTotalsResult.data;
...
export const processCheckoutSessionCompleted = async ({
  ...
  try {
    ...
    // 2. 在庫数と売り上げ数の更新
    const { success: updateStockSuccess, error: updateStockError } =
    ...
    // 4. 注文完了メールの送信
    const { success: orderEmailSuccess, error: orderEmailError } =
    ...
    // 5. 未払いの場合のメール送信
    if (
      checkoutSessionEvent.payment_status !== "paid" &&
      isNotEventSubscription
    ) {
    ...
    if (checkoutSessionEvent.payment_link && isNotEventSubscription) {
      const { success: deactivateSuccess, error: deactivateError } =
    ...
    console.error(
      "Database : Error in processCheckoutSessionCompleted: ",
      error,
    );
