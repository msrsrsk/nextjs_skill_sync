...
export const validateCartItem = (cartItem: CartItemWithProduct) => {
  ...
  const priceId =
    product.product_stripes?.sale_price_id ||
...
export const processCartItem = async ({
  ...
  try {
    ...
    if (price.unit_amount === null) {
      console.error(
        "Actions Error - Price unit_amount is null for price ID:",
        priceId,
      );
...
export const validatePriceCalculation = ({
  ...
  if (clientCalculatedTotal !== undefined) {
    if (
      clientCalculatedTotal !== serverCalculatedTotal ||
      clientCalculatedTotal !== dbBasedTotal
    ) {
      console.error(
        "Actions Error - Amount total mismatch:",
        clientCalculatedTotal,
        serverCalculatedTotal,
        dbBasedTotal,
      );
    ...
    if (serverCalculatedTotal !== dbBasedTotal) {
      console.error(
        "Actions Error - Amount total mismatch (server vs db):",
        serverCalculatedTotal,
        dbBasedTotal,
      );
      return false;
