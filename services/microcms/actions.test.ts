...
const { SYNC_UPDATES, SYNC_VOICES, SYNC_EXTRAS, SYNC_ALL } =
  SYNC_LOG_CATEGORIES;
...
describe("getAllSyncLogLists", () => {
  ...
  it("should return all sync log lists successfully", async () => {
    ...
    const mockUpdatesLogs = {
      ...
      error: null,
      data: {
        logList: [{ id: "log-3", title: "Updates Log 1", category: "Updates" }],
        totalCount: 1,
      },
    };
    const mockVoicesLogs = {
      ...
      error: null,
      data: {
        logList: [{ id: "log-4", title: "Voices Log 1", category: "Voices" }],
        totalCount: 1,
      },
    };
    const mockExtrasLogs = {
      ...
      error: null,
      data: {
        logList: [{ id: "log-5", title: "Extras Log 1", category: "Extras" }],
        totalCount: 1,
      },
    };
    ...
    mockClientGet
      .mockResolvedValueOnce({
        contents: mockAllLogs.data.logList,
        totalCount: mockAllLogs.data.totalCount,
      })
      .mockResolvedValueOnce({
        contents: mockUpdatesLogs.data.logList,
        totalCount: mockUpdatesLogs.data.totalCount,
      })
      .mockResolvedValueOnce({
        contents: mockVoicesLogs.data.logList,
        totalCount: mockVoicesLogs.data.totalCount,
      })
      .mockResolvedValueOnce({
        contents: mockExtrasLogs.data.logList,
        totalCount: mockExtrasLogs.data.totalCount,
      });
...
describe("getPaginatedSyncLogByCategory", () => {
  ...
  it("should return paginated data for last page", async () => {
    ...
    const mockData = {
      contents: [{ id: "log-5", title: "Log 5", category: "Extras" }],
      totalCount: 11,
    };
  ...
  it("should return paginated data for All category", async () => {
    ...
    const mockData = {
      contents: [{ id: "log-1", title: "Log 1", category: "All" }],
      totalCount: 1,
    };
