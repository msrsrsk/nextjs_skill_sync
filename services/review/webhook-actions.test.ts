...
const { WEBHOOK_INSERT_FAILED, WEBHOOK_DELETE_FAILED, WEBHOOK_PROCESS_FAILED } =
  REVIEW_ERROR;
...
describe("processReviewWebhook", () => {
  ...
  it("should process review webhook successfully with insert", async () => {
    const mockReceiveReviewNotificationEmail = vi.mocked(
      receiveReviewNotificationEmail,
    );
