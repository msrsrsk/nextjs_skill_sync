...
export async function sendSubscriptionPaymentRequestEmail({
  ...
  const shippingFee = formatNumber(
    Number(orderData.metadata?.subscription_shipping_fee) ?? 0,
  );
  const formattedOrderDate = formatOrderDateTime(orderData.created, DATE_SLASH);
  const subtotal = formatNumber(
    orderData.items.data[0]?.price?.unit_amount ?? 0,
  );
  ...
  try {
    ...
    console.error(
      "Email Error: Send Subscription Payment Request Email error:",
      error,
    );
