...
describe("receiveContactEmail", () => {
  ...
  it("should send contact email successfully with files", async () => {
    ...
    const mockFile1 = new File([file1Content], "document1.pdf", {
      type: "application/pdf",
    });
    const mockFile2 = new File([file2Content], "image1.jpg", {
      type: "image/jpeg",
    });

    const mockArrayBuffer1 = vi
      .fn()
      .mockResolvedValue(new TextEncoder().encode(file1Content).buffer);
    const mockArrayBuffer2 = vi
      .fn()
      .mockResolvedValue(new TextEncoder().encode(file2Content).buffer);
