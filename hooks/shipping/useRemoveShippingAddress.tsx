...
const useRemoveShippingAddress = ({
  ...
  const [removeAddressId, setRemoveAddressId] =
    useState<ShippingAddressId | null>(null);
  ...
  const removeShippingAddress = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${SHIPPING_ADDRESS_API_PATH}?addressId=${removeAddressId}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (result.success) {
        setOptimisticOtherAddresses(
          optimisticOtherAddresses.filter(
            (address) => address.id !== removeAddressId,
          ),
        );
