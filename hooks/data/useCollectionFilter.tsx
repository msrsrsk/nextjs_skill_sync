...
const useCollectionFilter = ({
  ...
  const getCurrentPriceRange = useCallback((): [number, number] => {
    const minPrice = parseInt(
      searchParams.get("minPrice") || priceBounds.minPrice.toString(),
    );
    const maxPrice = parseInt(
      searchParams.get("maxPrice") || priceBounds.maxPrice.toString(),
    );
  ...
  const [priceRange, setPriceRange] =
    useState<[number, number]>(getCurrentPriceRange);
