...
import { updateDefaultShippingAddressAction } from "@/services/shipping-address/form-actions";
...
const useUpdateDefaultShippingAddressForm = (
  ...
  const [updateDefaultState, updateDefaultFormAction] = useFormState(
  ...
  const [optimisticDefaultAddress, setOptimisticDefaultAddress] =
    useState<ShippingAddress | null>(defaultShippingAddress as ShippingAddress);
  const [editDefaultModalActive, setEditDefaultModalActive] = useState(false);

  const isDefaultAddressChanged = useCallback(
    (formData: FormData) => {
      ...
      return (
        optimisticDefaultAddress?.name !== name ||
  ...
  const handleDefaultAddressUpdate = useCallback(
    (formData: FormData) => {
