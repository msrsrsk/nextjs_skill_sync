...
const useSetDefaultShippingAddressForm = ({
  ...
  const [setDefaultState, setDefaultFormAction] = useFormState(
  ...
  useEffect(() => {
    if (setDefaultState.success && setDefaultState.timestamp) {
      const newDefaultAddress = optimisticOtherAddresses.find(
        (addr) => addr.id === addressId,
      );

      if (newDefaultAddress) {
        const newOtherAddresses = optimisticOtherAddresses.filter(
          (addr) => addr.id !== addressId,
        );
