...
const { SYNC_CONDITIONS, SKILL_TRAIL, OPTIMAL_SYNCS, CART } =
  GET_PRODUCTS_PAGE_TYPES;
const {
  CREATED_DESCENDING,
  BEST_SELLING,
...
export const getProductRepository = () => {
  return {
    // 商品詳細ページ:商品データの取得
    getProductBySlug: async ({ slug }: { slug: ProductSlug }) => {
    ...
    getProductByProductId: async ({ productId }: { productId: ProductId }) => {
      return await prisma.product.findUnique({
        where: { id: productId },
        select: {
          title: true,
    ...
    getProductBySlugForMetadata: async ({ slug }: { slug: ProductSlug }) => {
      return await prisma.product.findUnique({
        where: { slug },
        select: {
          title: true,
    ...
    getProductsByIds: async ({ ids, pageType }: GetProductsByIdsProps) => {
        ...
        categories.map(async (category) => {
          ...
          const [products, totalCount] = await Promise.all([
            prisma.product.findMany({
              ...
              take: limit,
              orderBy: [{ product_sales: { sold_count: "desc" } }],
            }),
      ...
      const whereCondition = {
        ...
        ...(query && {
          OR: [
            ...
            ...(isValidProductCategory(query)
              ? [{ category: query as CategoryType }]
              : []),
          ],
        }),
        ...(isTrend && {
