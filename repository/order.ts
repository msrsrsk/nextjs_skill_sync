...
const { CATEGORY_ALL, CATEGORY_NOT_SHIPPED, CATEGORY_SHIPPED } =
  ORDER_HISTORY_CATEGORIES;
...
export const createOrderRepository = () => {
  return {
    // 注文データの作成
    createOrder: async ({ orderData }: { orderData: CreateOrderData }) => {
...
export const getOrderRepository = () => {
  return {
    ...
    // 注文履歴の商品情報（IDと数量）の取得
    getOrderByIdWithOrderItems: async ({ orderId }: OrderIdProps) => {
      return await prisma.order.findUnique({
        where: { id: orderId },
        include: {
          order_items: {
    ...
    // 未発送の注文数の取得
    getUnshippedOrdersCount: async ({ userId }: UserIdProps) => {
...
export const deleteOrderRepository = () => {
  return {
    // 注文履歴の削除
    deleteOrder: async ({ orderId }: OrderIdProps) => {
