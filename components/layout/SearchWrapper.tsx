...
const SearchWrapper = async ({ page, query = "" }: SearchWrapperProps) => {
  ...
  const { products, totalPages, currentPage, hasNextPage, hasPrevPage } = data;
  ...
  return (
    <>
      {query && (
        <PageCountText
          countText={`「${query}」の検索結果`}
          customClass="mb-3 md:mb-4"
        >
          <span className="page-numtext-num">{products.length}</span>件
        </PageCountText>
      )}
