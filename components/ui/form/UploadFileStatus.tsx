...
const UploadFileStatus = ({
  ...
  const getStatusIcon = (type: string) => {
    switch (type) {
      case FILE_LOADING:
        return (
          <LoaderCircle className="w-[18px] h-auto animate-spin text-tag-extras mt-2" />
        );
      case FILE_SUCCESS:
  ...
  return (
    <>
      ...
      {fileStatuses
        ...
        .map((status, index) => (
          <li key={`${status.file.name}-${index}`} className="file-status-list">
            <div className="flex gap-2">
              ...
              <div>
                ...
                {status.type === FILE_LOADING && (
                  <p
                    className={`text-xs leading-[18px] font-bold ${getStatusTextClass(status.type)}`}
                  >
