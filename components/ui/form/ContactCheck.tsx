      ...
      {loading ? (
        ...
        <div>
          <div className="bg-soft-white rounded-sm pt-6 px-5 pb-7 md:px-10 grid gap-5 md:gap-7">
            <div>
              <h3 className="contact-check-label">お名前</h3>
              <p className="contact-check-text">
                {lastName} {firstName}
              </p>
            </div>
            <div>
              <h3 className="contact-check-label">メールアドレス</h3>
              <p className="contact-check-text">{email}</p>
            </div>
            <div>
              <h3 className="contact-check-label">お問い合わせ内容</h3>
              <p className="contact-check-text">{textarea}</p>
            </div>
            <div>
              <h3 className="contact-check-label">添付ファイル</h3>
              <div>
                {files?.length > 0 ? (
                  files.map((file) => (
...
const SubmitButton = () => {
  ...
  return (
    <EventButtonPrimary type={SUBMIT_TYPE} text={BUTTON_JA} disabled={pending}>
