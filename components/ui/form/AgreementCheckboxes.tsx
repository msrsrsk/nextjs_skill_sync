...
const AgreementCheckboxes = ({
  ...
  return (
    <div
      className={`bg-soft-white rounded-sm pt-6 px-5 pb-7 md:pt-10 md:px-[64px] md:pb-11 grid gap-4 md:gap-6${
      ...
      {checkList.map((list, index) => (
        <>
          <div key={list.id} className="flex items-start">
            ...
            />
            <label htmlFor={list.id} className="cursor-pointer pr-3 md:pr-6">
              <CheckIcon
          ...
          {index < checkList.length - 1 && <hr className="separate-border" />}
        </>
      ))}
    </div>
  );
