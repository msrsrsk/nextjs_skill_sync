...
const DropZone = ({
  ...
  const { fileStatuses, handleFileUpload, removeFile, removeErrorStatus } =
  ...
  const acceptedFormats =
    type === DROPZONE_REVIEW ? "JPG/JPEG/PNG" : "JPG/JPEG/PNG/PDF";
    ...
    <>
      <div
        className={`border-dashed border-2 rounded-[8px] pt-5 pb-6 flex flex-col items-center justify-center${
          isDragActive
            ? " border-tag-extras bg-[rgba(var(--tag-extras-rgb),0.1)]"
            : " border-form-line bg-form-bg"
        }`}
        {...getRootProps()}
      >
