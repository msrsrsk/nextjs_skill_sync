...
const SyncLogContent = ({
  ...
  const { logList, totalPages, currentPage, hasNextPage, hasPrevPage } =
    categoryData;
  ...
  return (
    <>
      <TabNavigation links={syncLogLinks} category={formattedCategory} />
        ...
        <>
          <div className="max-w-2xl mx-auto mr-5 md:mr-auto">
            <div className="border-b border-sub">
              {logList.map(
                ({ id, thumbnail, title, content, category, createdAt }) => (
                  <SyncLogCard
                    key={id}
                    id={id}
                    thumbnail={thumbnail}
