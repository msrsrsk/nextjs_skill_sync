...
const SupportChatContent = ({ chats, user }: SupportChatContentProps) => {
  ...
  const { chatMessages, loading, error, sendMessage, isChatLimitReached } =
    useChatSend({ chats });
  ...
  const isAIResponse = (chat: ChatProps) => {
    return (
      chat.sender_type === SENDER_ADMIN &&
      (chat.source === RULE_BASED ||
        chat.source === EMBEDDING_SEARCH ||
        chat.source === STAFF_CONFIRMING)
    );
  };
  ...
  return (
    <>
      <p className="text-sm leading-[26px] font-medium text-center mb-4">
        ※毎月 {CHAT_HISTORY_DELETE_MONTH} 日に過去のチャットデータは
              ...
              {chatMessages.map((chat, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`chat-message-textwrap${
                        isUser ? " justify-end" : ""
                      }`}
                    >
                      {isAdmin && (
                      ...
                      <div
                        className={`chat-message-textbox${
                      ...
                      {index !== 0 && (
                        <p
                          className={`chat-message-label font-semibold font-poppins${
                            isUser ? " text-right" : ""
                          }`}
                        >
