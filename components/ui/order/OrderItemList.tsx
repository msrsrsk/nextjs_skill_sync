        ...
        return (
          <div key={index} className="flex items-center gap-4 md:gap-6">
            <div className="max-w-[80px] min-w-[80px] md:w-full">
              <Link
                href={`${CATEGORY_PATH}/${product.category.toLowerCase()}/${product.slug}`}
              >
                <Image
            ...
            <div>
              ...
              <div className="product-list-pricebox">
                <p aria-label={`価格:${formatNumber(unit_price)}円（税込）`}>
                  <span
                  ...
                  <span className="product-list-price" aria-hidden="true">
                    <span
                      className={`${isRegularSale ? "text-tag-default" : ""}`}
                    >
                      {formatNumber(unit_price)}
                    </span>
                    <span className="ml-1 text-sm"> / </span>
                    <span className="text-sub text-sm">数量: {quantity}点</span>
                  </span>
                </p>
              </div>
              {remarks && (
