...
const OrderCard = ({ order }: { order: OrderPagenatedData }) => {
  const { created_at, status, order_items, order_number, total_amount } = order;
        ...
        <div className="w-full">
          <dl className="grid gap-1 md:gap-2">
            <div className="flex gap-[14px] md:gap-5">
              ...
              <dd className="order-card-infodd font-poppins">{order_number}</dd>
            </div>
            <div className="flex gap-[14px] md:gap-5">
            ...
            <div className="flex gap-[14px] md:gap-5">
              ...
              <dd className="text-base leading-[20px] font-medium">
                <p aria-label={`価格:${formatNumber(total_amount)}円（税込）`}>
                  <span className="product-list-symbol" aria-hidden="true">
                    ¥
                  </span>
                  <span
...
const MoreButton = ({ order }: { order: OrderPagenatedData }) => (
  <LinkButtonPrimary
