...
const CartWithItems = ({ cartItems, setCartItems }: CartWithItemsProps) => {
  ...
  useEffect(() => {
    if (cartRemoveSuccess) {
      ...
      const removedQuantity =
        optimisticQuantities[selectedItem] || itemToRemove.quantity;

      setCartItems(
        cartItems.filter((item) => item.product.id !== selectedItem),
      );
