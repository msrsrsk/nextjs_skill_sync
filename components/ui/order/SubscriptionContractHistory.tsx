...
const SubscriptionContractHistory = ({
  ...
  const subscriptionPayments =
    order_items[0].order_item_subscriptions?.subscription_payments;
  const maxItems = isMobile ? MOBILE_MAX_ITEMS : DESKTOP_MAX_ITEMS;
  const shouldShowOmit = subscriptionPayments.length > maxItems;

  const { getDisplayData, getCheckIconColor, getPaymentSubtext } =
  ...
  return (
    <div className="relative bg-form-bg rounded-[20px] lg:max-w-[440px] w-full py-5 px-6 grid gap-3">
      <p className="text-sm leading-[30px] font-bold md:text-base">契約履歴</p>
      <div className="grid gap-[10px]">
        {displayData.map(
          ({ payment: subscriptionPayment, originalIndex }, index) => (
            <div
