...
const ProductCategoryWrapper = async ({
  ...
  const categoryType = isAll ? undefined : (categoryTitle as CategoryType);

  const productRepository = getProductRepository();
  const { data: priceBoundsData } =
    await productRepository.getProductPriceBounds();
  ...
  const minPrice = parseInt(
    searchParams.minPrice || priceBounds.minPrice.toString(),
  );
  const maxPrice = parseInt(
    searchParams.maxPrice || priceBounds.maxPrice.toString(),
  );
  ...
  if (!getPaginatedProductsResult.success)
    return <ErrorMessage message={errorMessage} />;
  if (!getPaginatedProductsResult.data)
    return <ErrorMessage message={errorMessage} />;
