...
const ProductCategoryContent = ({
  ...
  const { collectionFilterOpen, setCollectionFilterOpen } =
    useCollectionFilterStore();
  const { collectionSortOpen, setCollectionSortOpen } =
    useCollectionSortStore();

  const { products, totalPages, currentPage, hasNextPage, hasPrevPage } =
    categoryData;
  ...
  return (
    <>
      <TabNavigation links={links} category={category} />
      ...
      {products.length === 0 ? (
        ...
        <>
          ...
          <div className="max-w-4xl mx-auto mr-5 md:mr-auto">
      ...
      <div
        className={`fixed bottom-5 md:bottom-10 left-1/2 -translate-x-1/2 p-[10px] md:py-3 md:px-5 flex gap-1 md:gap-4 z-[20] transition-all duration-300 ease-in-out${
