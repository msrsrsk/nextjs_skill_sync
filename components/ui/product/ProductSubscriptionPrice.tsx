...
const ProductSubscriptionPrice = ({
  ...
  const showOriginalPrice =
    !isSoldOut && salePrice && subscriptionPurchaseType === ONE_TIME;
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex flex-row items-center flex-wrap gap-2 md:gap-3">
        ...
        >
          <span className="price-text pr-[2px] text-lg" aria-hidden="true">
            ¥
          </span>
          <span className="price-text text-2xl" aria-hidden="true">
            {!isSoldOut && salePrice
        ...
        {productStatus && (
          <ProductStatus status={productStatus} size={STATUS_MEDIUM} />
        )}
      </div>
    </div>
  );
