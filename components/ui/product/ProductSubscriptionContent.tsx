...
const ProductSubscriptionContent = ({
  ...
  const validSubscriptionOptions =
    subscriptionOptions?.filter(
      (option) => option.priceId !== undefined && option.price !== undefined,
    ) || [];
  ...
  const {
    ...
    product,
    subscriptionOptions: validSubscriptionOptions,
  });
  ...
  return (
    <div className="mb-6 md:mb-8">
      <div className="flex gap-[10px] md:gap-5">
        <label
          className={`subscription-label${subscriptionPurchaseType === SUBSCRIPTION ? " is-active" : ""}`}
        >
          <input
            type="radio"
            value={SUBSCRIPTION}
        ...
        <label
          className={`subscription-label${subscriptionPurchaseType === ONE_TIME ? " is-active" : ""}`}
        >
          <input
            type="radio"
            value={ONE_TIME}
