...
import {
  ...
  optimalSyncsSliderSettings,
} from "@/lib/config/sliderOptions";
...
type ProductSliderContainerProps<T extends boolean> = {
  products: T extends true
    ? OptimalSyncsProductIds
    : ProductWithReviewsAndPricing[];
  totalCount: number;
  buttonLink?: string;
  isOptimalSyncs: T;
  customClass?: string;
...
const ProductSliderContainer = <T extends boolean = false>({
  ...
  const { isSliderInitialized, isSliderValid } = useSliderValidation(
  ...
  const { isDragging, handleDragStart, handleDragEnd } =
    useSliderDragDetection();

  if (!isSliderInitialized) return <LoadingSpinner />;

  const renderProductCards = () =>
    (products as ProductWithReviewsAndPricing[])?.map((product) => (
      <div key={product.id}>
        <ProductCard
          product={product}
          linksRef={linksRef}
  ...
  const renderAllOptimalSyncCards = (
    ...
    return allCards.map((card, index) => <div key={index}>{card}</div>);
  };

  return (
    <div
      className={`slider-product-container w-[100vw]${
    ...
    >
      ...
      {!isSliderValid ? (
        <div className="max-w-4xl mx-auto">
          <div
            className={`grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl-3xl:grid-cols-5 gap-[10px]${isOptimalSyncs ? " xl:gap-5" : " xl:gap-10"}`}
          >
            {isOptimalSyncs
              ? renderAllOptimalSyncCards(
