...
const ProductPurchaseButton = ({
  ...
  const isSubscriptionPurchase =
    isSubscription && subscriptionPurchaseType === SUBSCRIPTION;
        ...
        <>
          <div className="p-2 border border-foreground rounded-[12px]">
            <button
              ...
              onClick={
                isSubscriptionPurchase ? createPaymentLink : handleAddToCart
              }
              disabled={isDisabled}
            >
          ...
          <FormErrorText
            errorList={paymentLinkError ? [paymentLinkError] : []}
          />
        </>
      ) : (
        <div className="p-2 border border-foreground rounded-[12px]">
          <Link
            href={`${LOGIN_PATH}?callbackUrl=${encodeURIComponent(pathname)}`}
          >
            <button
