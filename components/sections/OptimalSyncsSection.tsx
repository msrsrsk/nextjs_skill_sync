...
const OptimalSyncsSection = async ({
  ...
  const { totalCount, requiredIds, optionIds, recommendedIds } = optimalSyncs;

  const repository = getProductRepository();

  const [requiredProducts, optionProducts, recommendedProducts] =
    await Promise.all([
      requiredIds.length > 0
        ? repository.getProductsByIds({
            ids: requiredIds,
  ...
  return (
    <section className="c-container-with-slider product-section">
      <SectionTitle title="Optimal Syncs" customClass="mb-6 md:mb-10" />
